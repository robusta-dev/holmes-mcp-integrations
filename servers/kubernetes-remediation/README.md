...
| `GPU_DIAG_DCGMI_PATH` | *(unset)* | optional absolute host path of `dcgmi`, tried before the host PATH |
...
**pinned to the target node** (via `spec.nodeName`, so a cordoned node can
still be diagnosed; a blanket toleration keeps GPU-node taints from evicting
it), labeled `robusta.dev/diagnostic-pod: "true"` like the other diagnostic
pods, with no ServiceAccount token, and auto-deleted afterwards.

**Everything runs on the host filesystem, using the node's own binaries** — no
custom or GPU-specific image. The pod (stock busybox: it only supplies a
shell) is privileged with `hostPID` and the host root mounted **read-only** at
`/host`, the same shape as `kubectl debug node/`. `nvidia-smi` is resolved
from the host PATH, falling back to `/run/nvidia/driver` for GPU Operator
clusters that run the containerized driver; `dmesg`, `journalctl`, `lspci`,
`modinfo` and `systemctl` run via `chroot /host` too.

The caller selects checks **by name** (several per call — they all run in the
same pod, one `===== <check> =====` section per check in the output). The
catalog covers the nvidia-smi surface (driver/temperature/power overview,
throttle reasons, ECC counts, retired pages / row remapping,
GPU-memory-holding processes, ~30s utilization sampling) and the kernel side
...
and link state, fabric-manager status, `/dev/nvidia*` holders, per-process
inspection).
...
read-only checks, never compose a command. Every command string is a
server-owned constant; the only caller-supplied values that reach one are a
